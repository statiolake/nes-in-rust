...
impl<'a> Pattern<'a> for char {
    ...
    #[inline]
    fn into_searcher(self, haystack: &'a str) -> Self::Searcher {
        let utf8_encoded = [self as u8, 0, 0, 0];
        let utf8_size = 1;
        CharSearcher {
            haystack,
            finger: 0,
...
struct TwoWaySearcher {
    ...
    /// it's a 32-bit "fingerprint" where each set bit `j` corresponds
    /// to a (byte & 31) == j present in the needle.
    byteset: u32,
...
impl TwoWaySearcher {
    ...
    #[inline]
    fn byteset_create(bytes: &[u8]) -> u32 {
        bytes.iter().fold(0, |a, &b| (1 << (b & 0x1f)) | a)
    }

    #[inline]
    fn byteset_contains(&self, byte: u8) -> bool {
        (self.byteset >> ((byte & 0x1f) as usize)) & 1 != 0
    }
